# Milestone v1.0: Production-Ready Platform

**Status:** ✅ SHIPPED 2026-01-27
**Phases:** 1-5
**Total Plans:** 11

## Overview

Transform DoppelDown from validated prototype to production-ready brand protection platform. This milestone hardens existing infrastructure, implements tier-based access controls, adds automated background scanning, and enables user-configured email alerts. Focus: reliability, enforcement, and automation for weekend sprint demo.

## Phases

### Phase 1: Admin Foundation

**Goal**: Critical infrastructure in place for tier enforcement and admin privileges
**Depends on**: Nothing (first phase)
**Requirements**: ADMN-01, ADMN-02, TIER-01, TIER-02
**Plans**: 1 plan

Plans:
- [x] 01-01-PLAN.md — Add admin column, fix brand limits, implement admin bypass

**Success Criteria:**
1. Admin accounts exist in database with `is_admin` column
2. Admin accounts bypass all tier limits (brand count, scan limits)
3. Free accounts cannot create brands beyond limit
4. Each tier respects its brand count limit (Free=1, Starter=3, Pro=10, Enterprise=unlimited)

**Details:**
- Added `is_admin` boolean column to users table with partial index
- Created BRAND_LIMITS constant with tier-specific limits
- Implemented `!userData?.is_admin &&` guard pattern for consistent bypass
- Fixed effectiveTier fallback logic for broken tier enforcement

### Phase 2: Scanning Hardening

**Goal**: Existing scans run reliably with graceful error handling and recovery
**Depends on**: Phase 1
**Requirements**: HARD-01, HARD-02, HARD-03, HARD-04
**Plans**: 3 plans

Plans:
- [x] 02-01-PLAN.md — Install p-queue, create scan-queue.ts and api-client.ts
- [x] 02-02-PLAN.md — Add progress schema, refactor scan-runner with queues
- [x] 02-03-PLAN.md — Integrate queues into scanners, create ScanProgress UI

**Success Criteria:**
1. Scans continue when individual API calls timeout or fail (no full scan crashes)
2. System respects external API rate limits with queuing (no 429 errors causing cascading failures)
3. Failed scans retry automatically with exponential backoff (no manual intervention needed)
4. User can see real-time scan progress percentage and cancel in-progress scans

**Details:**
- Created scan-queue.ts with provider-specific rate limits (DNS 10/s, search 2/s, screenshot 1/2s, OpenAI 50/min)
- Added progress_percentage, current_step, step_details to scans table
- Refactored scan-runner with weighted step progress and partialErrors array
- Created ScanProgress component with polling, retry badge, cancel button

### Phase 3: Manual Scan Limits

**Goal**: Tier-based manual scan quotas enforced to drive upgrades
**Depends on**: Phase 1 (tier enforcement infrastructure), Phase 2 (reliable scans)
**Requirements**: SCAN-03, SCAN-04, TIER-03
**Plans**: 2 plans

Plans:
- [x] 03-01-PLAN.md — Add quota schema, API enforcement, quota status endpoint
- [x] 03-02-PLAN.md — Update scan button UI with quota display and upgrade CTA

**Success Criteria:**
1. Free tier users limited to 1 manual scan per week (upgrade wall)
2. Paid tier users get unlimited manual scans
3. Manual scan button shows quota status ("X scans remaining" for Free tier)
4. System tracks manual scan count per user per tier period

**Details:**
- Added manual_scan_count, manual_scan_reset_at columns to users table
- Created getManualScanLimit() helper returning null for unlimited
- Rolling 7-day window from first scan (not calendar week)
- useQuotaStatus hook with blue upgrade banner for helpful upsell tone

### Phase 4: Automated Scanning

**Goal**: Background job scheduler running tier-appropriate scan frequencies
**Depends on**: Phase 2 (hardened scans), Phase 3 (scan limit tracking)
**Requirements**: SCAN-01, SCAN-02
**Plans**: 2 plans

Plans:
- [x] 04-01-PLAN.md — Add auto_scan_enabled column, update tier-limits with hours-based frequency
- [x] 04-02-PLAN.md — Update cron endpoint with hours-based scheduling, jitter, and pause filtering

**Success Criteria:**
1. Starter tier brands scanned daily at configured time
2. Pro tier brands scanned every 6 hours
3. Enterprise tier brands scanned hourly
4. Free tier brands never auto-scanned (manual only)
5. Background jobs respect brand-level scan preferences (pause, schedule override)

**Details:**
- Added auto_scan_enabled boolean to brands table (default true)
- Created scanFrequencyHours in TIER_LIMITS (null/24/6/1)
- Updated /api/cron/scan with hours-based scheduling and 0-5 minute jitter
- Partial index on (last_scan_at DESC) for cron optimization

### Phase 5: Alert Settings

**Goal**: Users control email notification preferences per threat severity
**Depends on**: Phase 4 (automated scanning generates alerts)
**Requirements**: ALRT-01, ALRT-02, ALRT-03, ALRT-04
**Plans**: 3 plans

Plans:
- [x] 05-01-PLAN.md — Add alert preference columns, update types, add email template functions
- [x] 05-02-PLAN.md — Update settings UI, create weekly digest cron endpoint
- [x] 05-03-PLAN.md — Wire sendScanSummary and severity_threshold in scan-runner (gap closure)

**Success Criteria:**
1. User can enable/disable email alerts in settings page
2. User can set severity threshold (all threats / high+critical / critical only)
3. User can toggle scan completion summary emails (sent after each scan)
4. User can toggle weekly digest emails (summarizing week's threats)
5. Alert preferences persist and apply to all future scans

**Details:**
- Added severity_threshold, scan_summary_email, weekly_digest columns
- Radio buttons for severity threshold (clearer than chip toggles)
- sendScanSummary() and sendWeeklyDigest() email functions
- Weekly digest cron Monday 9am UTC
- Fallback chain for backward compatibility with existing alert_on_severity

---

## Milestone Summary

**Key Decisions:**
- Admin bypass uses `!userData?.is_admin &&` guard pattern (consistent, greppable, null-safe)
- Enterprise tier uses Number.MAX_SAFE_INTEGER for unlimited (semantic clarity)
- Provider-specific queues with conservative limits (no 429 cascades)
- Weighted step progress (domains 40%, web 25%, logo 15%, social 20%)
- Rolling 7-day window for quota (not calendar week)
- Hours-based scan frequency with deprecated days field kept for compat
- Severity threshold 'high_critical' as default

**Issues Resolved:**
- Free tier bypassing all limits (TIER-01) — fixed with effectiveTier fallback
- No rate limiting (HARD-02) — p-queue with provider-specific limits
- Manual scan quota not enforced — backend + UI with upgrade CTA
- Alert preferences not wired — gap closure plan 05-03 addressed

**Issues Deferred:**
- Pre-existing TypeScript errors (pages_scanned, screenshot_url, downlevelIteration)
- OpenAI Vision endpoint uses wrong API path
- api-client.ts not wired to all scanners (optional enhancement)

**Technical Debt Incurred:**
- None — all phases executed as planned with gap closure

---

_For current project status, see .planning/ROADMAP.md_
